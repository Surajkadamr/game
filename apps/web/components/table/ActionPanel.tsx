...
export function ActionPanel({
  ...
  // Force-close the raise sub-panel so it doesn't persist stale state
  useEffect(() => {
    if (!isVisible) {
      setShowRaise(false);
    }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Client-side safety: if timer runs out and we haven't acted, auto-act
      // The server will have already auto-acted, but this clears the UI cleanly
      if (remaining <= 0 && !actionSentRef.current) {
        actionSentRef.current = true;
        // Don't send another action (server already handled it), just clear UI
  ...
  const handleAction = useCallback((action: ActionType, amount?: number) => {
    if (actionSentRef.current) return; // deduplicate
    actionSentRef.current = true;
  ...
  const handleSliderChange = useCallback((value: number) => {
    const snapped = snapToStep(value, 1000); // snap to ₹10
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-xl z-50"
        ...
        >
          ...
          <div
            className="glass-dark rounded-t-2xl p-4 border-t border-x border-white/10"
            style={{ background: 'rgba(5, 8, 16, 0.94)' }}
          >
            {/* Header */}
              ...
              <div className="flex items-center gap-3">
                {callAmount > 0 && (
                  <span className="text-xs text-white/40">
                    To call: {formatRupees(callAmount)}
                  </span>
                )}
                <span
                  className={cn(
            ...
            {/* Action Buttons */}
            <div className={cn('grid gap-2', showRaise ? 'grid-cols-1' : 'grid-cols-3')}>
              {!showRaise && (
                <>
                  <button
                    onClick={handleFold}
                    className="btn-fold py-3 rounded-xl font-bold text-sm uppercase tracking-wider"
                  >
                    ✕ Fold
                  </button>
                  ...
                  <button
                    ...
                    className="btn-check py-3 rounded-xl font-bold text-sm uppercase tracking-wider"
                  >
                    {canCheck ? '✓ Check' : `Call ${formatRupees(callAmount)}`}
                  </button>

                  <button
                    onClick={() => setShowRaise(true)}
                    className="btn-raise py-3 rounded-xl font-bold text-sm uppercase tracking-wider"
                    disabled={maxRaise <= 0}
                  >
                    ↑ Raise
                  </button>
              ...
              {showRaise && (
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button
                    ...
                    className="py-3 rounded-xl font-bold text-sm text-white/40 glass hover:glass-dark transition-all uppercase tracking-wider"
                  >
                    ← Back
                  </button>
                  <button
